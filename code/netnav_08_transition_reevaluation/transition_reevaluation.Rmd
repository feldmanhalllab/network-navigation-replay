...
We'll start by looking at study 2. Note that in other disaggregated analyses, we don't estimate a random slope for shortest path distance due to convergence issues. To maintain consistency, we'll omit that random slope in these analyses also.
...
stats_nav_study2_tr_dist2 <- nav_study2 %>%
  ...
  glmmTMB(
    correct ~ diff_dist + measurement_id * shortest_path +
      (1 + measurement_id | sub_id),
    family = binomial,
    data = .
  )
...
map_dfr(
  ...
  select(-c(effect, component)) %>%
  kable_custom(
    "Navigation accuracy: Day 2b, Study 2", grouping_var = ref_cat, digits = 3
  )
```
...
map_dfr(
  ...
  select(-c(effect, component)) %>%
  kable_custom(
    "Navigation accuracy: Day 2b, Study 3", grouping_var = ref_cat, digits = 3
  )
```
