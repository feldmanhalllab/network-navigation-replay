...
softmax <- function(
    option_values, option_chosen,
    temperature = NULL, use_inverse_temperature = FALSE,
    lapse_rate = 0
) {
  ...
  if (lapse_rate < 0 | lapse_rate > 1) {
    stop("Lapse rate must be bounded in [0, 1].")
  }
  
  # Standard Gibbs / Boltzmann distribution
  if (use_inverse_temperature) {
  ...
  # Lapse rate for irreducible noise
  # https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3387384/
  p_choice <- (numerator / denominator) *
    (1 - lapse_rate) +
    (lapse_rate / length(option_values))
