...
```{r load-params}
params <- here("data", "model_confusion") %>%
  fs::dir_ls(
...
if (knitting) {
  ggsave(
    filename = here("outputs", workflow_name, "model_confusion.pdf"),
...
if (knitting) {
  ggsave(
    filename = here("outputs", workflow_name, "model_inversion.pdf"),
...
if (knitting) {
  ggsave(
    filename = here("outputs", workflow_name, "pxp.pdf"),
    plot = plot_pxp,
    width = 5, height = 2.5,
    units = "in", dpi = 300
  )
...
In the supplement, we'll want a plot of both the confusion and inversion matrices.

```{r plot-model-recovery-for-supp}
plot_confusion_for_supp <- wrap_plots(
  plot_confusion_matrix,
  plot_inversion_matrix,
  nrow = 1
) +
...
plot_confusion_for_supp

if (knitting) {
  ggsave(
    filename = here("figures", "supp_model_recovery.pdf"),
    plot = plot_confusion_for_supp,
    width = 8, height = 4,
    units = "in", dpi = 300
  )
}
