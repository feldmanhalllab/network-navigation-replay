...
set.seed(sum(utf8ToInt("Look on my works, ye mighty, and despair!")))
...
bfs_sims <- mpt_data %>%
  ...
  mutate(
    test = map(
      .x = data,
      .f = ~mpt_bfs_reverse(
        g, .x$startpoint_id, .x$endpoint_id, .x$opt1_id, .x$opt2_id
      )
    )
  ) %>%
  unnest(test) %>%
...
if (run_on_cluster) {
  ...
  bfs_sims %>%
    ...
    write_csv(here("data", "bfs_sims", "bfs_sims_reevaluated_backward.csv"))
}
